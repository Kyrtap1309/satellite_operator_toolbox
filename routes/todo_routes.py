...
def add_subtask() -> tuple[Response, int] | Response:
    ...
    try:
        ...
        # Check if subtask should be scheduled
        ...
        if scheduled:
            ...
            if start_date and start_time_str and end_date and end_time_str:
                try:
        ...
        success = todo_service.add_subtask(task_id, title, description, start_time, end_time)
        if success:
            flash("Subtask added successfully!", "success")
        else:
            flash("Failed to add subtask", "error")

    except Exception as e:
        logger.error(f"Error adding subtask: {e}")
        flash("An error occurred while adding subtask", "error")
